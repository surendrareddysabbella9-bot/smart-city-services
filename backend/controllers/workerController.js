...
export const getWorkers = async (req, res, next) => {
  ...
  try {
    let selectClause = `
      w.id, u.name, w.category, w.experience, w.location, w.verification_status,
    ...
    for (let worker of workers) {
      ...
      if (worker.distance) {
        worker.distance = parseFloat(worker.distance).toFixed(2); // Convert to km with 2 decimals
      }
    }
    
    // If no distance sorting was done in SQL, keep the MatchScore sorting
    if (!lat || !lng) {
